...
The phase space factor ρ(m) includes proper analytic continuation below threshold
through the phase rotation parameter `ϕ`.

# Arguments
- `ch::TwoBodyChannel`: Two-body channel definition
- `m::Number`: Mass where to evaluate phase space
- `ϕ::Number=-π/2`: Phase rotation parameter for analytic continuation
